...
export class AppointmentService {
  ...
  async rescheduleAppointment(id: string, data: any, user: any) {
    const appointment = await this.appointmentRepository.findOne({ where: { id }, relations: ['patient', 'patient.user', 'doctor', 'elasticSchedule'] });
    ...
    // If appointment is for elastic slot
    if (appointment.elasticSchedule) {
      const elasticSchedule = appointment.elasticSchedule;
      // Calculate available slots for the doctor on that date
      const slotDuration = elasticSchedule.slotDuration;
      const buffer = elasticSchedule.bufferTime || 0;
      const start = elasticSchedule.startTime;
      const end = elasticSchedule.endTime;
      ...
      // If patient hasn't picked a slot yet, return available slots
      if (!data.startTime || !data.endTime) {
        return { availableSlots };
      }
      // Validate selected slot
      const selected = availableSlots.find(s => s.startTime === data.startTime && s.endTime === data.endTime);
      if (!selected) throw new ConflictException('Selected slot is not available');
